    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      {(race.country ||
        ...
        <p className="text-gray-600 mb-4">
          {[race.country, race.type, race.distances?.join(", ")]
            .filter(Boolean)
            .join(" • ")}
        </p>
      )}
      ...
      {(race.date || race.price || race.saleStatus) && (
        <div className="space-y-2 mb-6">
          {race.date && (
            <p>
              <span className="font-semibold">Data:</span> {race.date}
            </p>
          )}
          {race.price && (
            <p>
              <span className="font-semibold">Preço:</span> {race.price}
            </p>
          )}
          {race.saleStatus && (
            <p>
              <span className="font-semibold">Status:</span> {race.saleStatus}
            </p>
          )}
        </div>
      )}
